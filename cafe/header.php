...
<script>
    $(function() {
        $("#discount, #ticketdiscount, #totalAmount").on("keydown keyup", sumT);
        
            function sumT() {
             // Get input values safely
              const discount = parseFloat($("#discount").val()) || 0;
              const ticketDiscount = parseFloat($("#ticketdiscount").val()) || 0;

              // Clean and parse hidden total
              const rawTotal = ($("#totalAmountHidden").val() || "0").replace(',', '');
              const totalItems = parseFloat(rawTotal) || 0;

              // === VAT and Discounts Calculation ===
              const vatExempt = discount / 1.12;
              const twentyPercent = vatExempt * 0.20; // 20% PWD/SC discount

              const vatSales = (totalItems - discount) / 1.12;
              const vat = vatSales * 0.12;

              const serviceCharge = totalItems * 0.10; // ✅ matches PHP (10%)

              // Final amount due:
              // vat sales + VAT + vat exempt + service charge - (20% discount + ticket discount)
              const amountDue = (vatSales + vat + vatExempt + serviceCharge) - (twentyPercent + ticketDiscount);


              // Update UI (always show 2 decimals)
              $("#totalAmount").val(amountDue.toFixed(2));
          }

       
       /* $("#discountBill, #ticketBill, #totalAmountBill").on("keydown keyup", sumTBill);
        
        function sumTBill() {
            let discountBill = parseFloat($("#discountBill").val()) || 0;
            let ticketBill = parseFloat($("#ticketBill").val()) || 0;
           // console.log($("#totalAmountHidden").val() +"<>"+$("#discount").val());
        $("#totalAmountBill").val(parseFloat($("#totalAmountHiddenBill").val().replace(',', '')) - discountBill - ticketBill);
    ...
    //}
        ...
        $("#cash_amount, #gcash_amount , #card_amount , #ent_amount").on("keydown keyup", sumA);
            ...
            function sumA() {
              // Parse payment inputs safely
              const cash  = parseFloat($("#cash_amount").val())  || 0;
              const gcash = parseFloat($("#gcash_amount").val()) || 0;
              const card  = parseFloat($("#card_amount").val())  || 0;
              const ent   = parseFloat($("#ent_amount").val())   || 0;

              // Compute totals
              const totalPayment = cash + gcash + card + ent;
              const totalAmount  = parseFloat($("#totalAmount").val()) || 0;
              const change       = totalPayment - totalAmount;

              // Update change field with 2 decimals
              $("#change").val(change.toFixed(2));

              // Enable/disable submit button based on payment sufficiency
              if (change >= 0) {
