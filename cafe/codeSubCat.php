...
$query = "SELECT * FROM product_subcategories WHERE name = '$name'";
$result = mysqli_query($conn,$query);
$row_name = $result->fetch_assoc();

if($del == 1){
    $query = "DELETE FROM product_subcategories  WHERE id='$id'";
        $result = mysqli_query($conn,$query);
        if($result)
        {
    ...
    if($id == ""){//add
        if($result-> num_rows == 0){
            $query = "INSERT INTO product_subcategories (name, category_id) VALUES ('$name', '$category')";
            $result = mysqli_query($conn,$query);
            if($result)
            {
        ...
        $query_id = "SELECT * FROM product_subcategories WHERE id = '$id'";
        ...
        if($row_name['name'] == $name){ //check if name exits in table
            ...
            $query = "UPDATE product_subcategories SET name = '$name', category_id = '$category'  WHERE id='$id'";
            $result = mysqli_query($conn,$query);
            if($result)
            {
